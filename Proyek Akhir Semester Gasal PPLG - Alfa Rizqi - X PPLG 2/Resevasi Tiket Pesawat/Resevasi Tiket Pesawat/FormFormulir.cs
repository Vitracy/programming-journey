...
namespace Resevasi_Tiket_Pesawat
{
    public partial class FormFormulir : Form
    {
        ...
        string[] namaPenerbangan = new string[] { "Citilink", "Lion Air", "Batik Air", "AirAsia" };
        ...
        private void btn_kirim_formulir_Click(object sender, EventArgs e) // jika tombol kirim di klik
        {
            ...
            if (double.TryParse(textBox_nomor_telepon.Text, out double nomor_telepon)) // mendeteksi kalau textBox_nomor_telepon.Text adalah angka, dan dimasukkan ke var nomor_telepon
            {
        ...
        private void button_hapus_tiket_Click(object sender, EventArgs e)
        {
            ...
            {
                int total_harga = Convert.ToInt32(label_total_harga.Text); // isi var total harga dengan label_total_harga.Text yang diubah menjadi int 
                total_harga -= Convert.ToInt32(dataGrid_ticket.CurrentRow.Cells["Total_Harga"].Value); // var total harga lalu di kurang nilai dari sel Total_Harga pada dataGrid_ticket row saat ini yang sudah diubah ke int
                label_total_harga.Text = $"{total_harga}"; // set label_total_harga.Text ke label_total_harga.Text 
                ...
                textBox_biaya_admin.Text = ""; // mengosongkan textbox biaya admin
                textBox_uang_pelanggan.Text = ""; // mengosongkan textbox uang pelanggan
                lbl_kembalian.Text = "Rp.  0"; // mengeset lbl kembalian text ke setelan awal (Rp. 0).
            }
            else
            {
